 ...
 */
public class QuadBufferGeometry extends BufferGeometry {

  public QuadBufferGeometry(Box2 UV, double width, double height) {
    this(new Vector3(), UV, width, height);
  }

  public QuadBufferGeometry(Vector3 position, Box2 UV, double width, double height) {
    this(Collections.singletonList(position), Collections.singletonList(UV), width, height);
  }

  public QuadBufferGeometry(Collection<Vector3> positions, Collection<Box2> UVs, double width, double height) {
  ...
  // The collection must be of same size as it was in the constructor! Else
  // errors will happen... we do not perform checks.
  public void updateUVs(Collection<Box2> UVs) {
    BufferAttribute uvsb = this.getAttribute("uv");
    Float32Array uvs = (Float32Array) uvsb.getArray();
    ...
    uvsb.setNeedsUpdate(true);
  }

  public void updateUV(Box2 UV) {
    updateUVs(Collections.singletonList(UV));
  }
}
